...
const registerUser = asyncHandler(async (req, res) => {
  ...
  console.log(
    `Avatar: ${avatarLocalPath}, coverImageLocalPath: ${coverImageLocalPath}`
  );

  if (!avatarLocalPath) {
    throw new ApiError(404, "Avatar file is not required");
  }
...
const logoutUser = asyncHandler(async (req, res) => {
  ...
  return res
    ...
    .json(new ApiResponse(401, {}, "User loggedOut successfully"));
});
...
const changeCurrentPassword = asyncHandler(async(req, res)=>{
    ...
    const isPasswordCorrect = user.isPasswordCorect(oldPassword)
...
const updateUserCoverImage = asyncHandler(async(req, res)=>{
    const coverLocalPath = req.user?.path;
...
export { registerUser, loginUser, logoutUser, refreshAccessToken, changeCurrentPassword, getCurrentUser, updateAccountDetails, updateUserAvatar, updateUserCoverImage, getCurrentUser };
